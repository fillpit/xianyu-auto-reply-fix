...
async def login(login_request: LoginRequest, request: Request):
    ...
    # 检查IP是否被封禁
    ip_blocked, ip_block_reason, _ = check_ip_blocked(client_ip)
    if ip_blocked:
    ...
    # 检查用户名是否被锁定
    if username:
        user_locked, user_lock_reason, _ = check_user_locked(username)
        if user_locked:
            logger.warning(f"🔒 用户 {username} 尝试登录但账户已锁定 (IP: {client_ip})")
    ...
    if username and login_request.password:
        logger.info(f"【{username}】尝试用户名登录 (IP: {client_ip})")

        if db_manager.verify_user_password(username, login_request.password):
        ...
        logger.warning(f"【{username}】登录失败：用户名或密码错误 (IP: {client_ip})")
        return LoginResponse(
            success=False,
            message="用户名或密码错误"
        )
